...
class _SurveyStartScreenState extends State<SurveyStartScreen> {
  ...
  String? _selectedProjectId; // null = 全部
  SurveyMode _surveyMode = SurveyMode.point;
  ...
  Future<void> _loadLastProject() async {
    ...
    if (!mounted) return;
    final id = prefs.getString('last_project_id');
    if (id == null) return;
    final prov = context.read<SurveyProvider>();
    if (prov.surveyProjects.any((p) => p.id == id)) {
  ...
  Future<void> _selectProject(String? id) async {
    setState(() {
      ...
      _didFitPoints = false; // trigger map refit for new project
    });
    final prefs = await SharedPreferences.getInstance();
    if (id == null) {
      await prefs.remove('last_project_id');
    } else {
      await prefs.setString('last_project_id', id);
    }
  }

  List<SurveyPoint> _displayedPoints(SurveyProvider prov) {
    final all = prov.visibleSurveyPoints;
    if (_selectedProjectId == null) return all;
    final proj = prov.surveyProjects.cast<SurveyProject?>().firstWhere(
      (p) => p?.id == _selectedProjectId,
      orElse: () => null,
    );
    if (proj == null) return all;
  ...
  Map<String, String> _collectValues(List<CustomField> fields) {
    ...
    if (_selectedPoint != null) {
      ...
      if (_selectedPoint!.county.isNotEmpty) {
        values['县市'] = _selectedPoint!.county;
      }
      if (_selectedPoint!.windFarm.isNotEmpty) {
        values['风电场'] = _selectedPoint!.windFarm;
      }
    }
    return values;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final displayed = _displayedPoints(prov);

    // nearby points sorted by distance, scoped to current project
    final nearby =
        pos != null
            ? prov.nearbyPoints(
              pos.latitude,
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
              ...
              children: [
                ...
                FlutterMap(
                  ...
                  children: [
                    ...
                    // Survey point markers (filtered by project)
                    MarkerLayer(
                      markers: [
                        ...displayed.map(
                          (p) => Marker(
                              ...
                              builder:
                                  (_) => _FullscreenMapPage(
                                    ...
                                    initialZoom:
                                        pos != null
                                            ? 13.0
                                            : (displayed.isEmpty ? 6.0 : 10.0),
                                    surveyPoints: displayed,
          ...
          // ── Project filter ────────────────────────────────────────────────
          if (prov.surveyProjects.isNotEmpty)
            _ProjectFilterRow(
              projects: prov.surveyProjects,
              allPoints: prov.visibleSurveyPoints,
            ...
            child: ListView(
              ...
              children: [
                  ...
                  ...fields.map(
                    (f) => Padding(
                      ...
                      child: _FieldWidget(
                        ...
                        onSelectChanged: (v) => setState(() => _sel[f.id] = v),
                      ),
                    ),
                  ),
                ],
                ...
                _LocationStatusBar(
                  ...
                  onClearManual: () => setState(() => _manualLocation = null),
                ),
                const SizedBox(height: 12),
                SegmentedButton<SurveyMode>(
                  segments: const [
                    ButtonSegment(
                      value: SurveyMode.point,
                      icon: Icon(Icons.place),
                      label: Text('样点法'),
                    ),
                    ButtonSegment(
                      value: SurveyMode.transect,
...
class _ProjectFilterRow extends StatelessWidget {
  ...
  final List<SurveyPoint> allPoints;
          ...
          children: [
            ChoiceChip(
              ...
              labelStyle: TextStyle(
                color:
                    selectedProjectId == null ? Colors.white : Colors.grey[800],
              ),
            ...
            ...projects.map((proj) {
              final count =
                  allPoints.where((p) => proj.pointIds.contains(p.id)).length;
              if (count == 0) return const SizedBox.shrink();
              final selected = selectedProjectId == proj.id;
              return Padding(
                ...
                child: ChoiceChip(
                  ...
                  onSelected: (_) => onSelected(selected ? null : proj.id),
                ),
              );
            }),
          ],
