...
1. 鸟种闪卡（中文名 / 学名 / 音频 / 图片）；
2. 鸟种浏览、收藏与统计；
3. 数据包管理（本地导入 / 在线下载 / 已有数据包）；
4. 媒体上传（普通用户可保存本地；仅受邀用户和管理员可同步至服务器）。

二、账户与角色

2.1 普通用户：无需注册即可使用大部分功能。上传的媒体仅保存到本机数据包。

2.2 受邀上传用户：由管理员分配上传 Token。上传内容进入审核队列，管理员审核通过后才会向其他用户公开；未获邀请的普通用户不能向服务器提交媒体。
...
3.2 **上传素材的权利声明**：服务器上传功能仅向受邀用户开放，并需经管理员审核。您确认拥有所上传素材的著作权或已获得权利人合法授权，并同意以 **CC BY-NC 4.0 协议**（署名-非商业性使用 4.0 国际，详见 https://creativecommons.org/licenses/by-nc/4.0/deed.zh-hans ）将其授权给本 App 及其他用户作非商业学习、研究和科普用途使用。我们将保留您的署名并标注来源。
...
- 来源标注为 birdaholic-upload（即受邀用户贡献）的，遵守 CC BY-NC 4.0；
- 详细许可信息见各物种页面"致谢"区。
