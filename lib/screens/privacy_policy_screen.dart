...
1.2 上传到服务器的内容（由您主动触发）
仅当您主动点击"上传"按钮时，下列内容会发送到我们的服务器（birding.today）：
- 您选择的照片 / 录音文件；
- 您填写的署名、描述、识别特征、难度评级；
...
| 照片 / 录音 + 署名/描述 | 您点"上传" | 公开到对应物种页 | 是 → 我方服务器 |
...
- 查看 / 修改本机数据：在"设置"中完成；
- **注销 / 解绑身份**：内测用户在"设置 → 上传 Token"中清空 Token，即视为解除身份绑定（注销）。我方 users.json 中的 Token 可应您要求撤销；
