#!/usr/bin/env python3
"""生成 region_names_zh.json：地区代码 -> 中文名。
来源：① 本文件内置的国家 ISO2->中文表；② 中国省级（CN-XX）来自 App 内置
provinces/_manifest.json。其余 subnational1 没有中文名，由 gen_checklists 用
eBird 英文名兜底。

用法（在仓库根目录跑）：
  python3 server/build_region_names.py            # 写到 server/region_names_zh.json
"""
import json
from pathlib import Path

# ISO 3166-1 alpha-2 -> 中文国家/地区名（eBird country code 用的就是 ISO2）
COUNTRY_ZH = {
    ...
    "MF": "圣马丁（法属）", "MG": "马达加斯加", "MH": "马绍尔群岛", "MK": "北马其顿",
    ...
    "ST": "圣多美和普林西比", "SV": "萨尔瓦多", "SX": "圣马丁（荷属）", "SY": "叙利亚",
    "SZ": "斯威士兰", "TC": "特克斯和凯科斯群岛", "TD": "乍得", "TF": "法属南方领地",
...
def main() -> None:
    ...
    out = {}
    out.update(COUNTRY_ZH)

    # 合并中国省级中文名（来自 App 内置 provinces 清单）
    manifest = repo / "assets/data/provinces/_manifest.json"
    if manifest.exists():
        m = json.loads(manifest.read_text(encoding="utf-8"))
        for p in m.get("provinces", []):
            code = p.get("code", "")
            name = p.get("name", "")
            if code and name:
                out[code] = name

    dest = repo / "server/region_names_zh.json"
    dest.write_text(json.dumps(out, ensure_ascii=False, indent=0), encoding="utf-8")
    print(f"写入 {dest}：{len(out)} 条（国家 {len(COUNTRY_ZH)} + 中国省 {len(out)-len(COUNTRY_ZH)}）")
